...
class ModeloDetalles{
	...
	static public function mdlRangoFechasDetalles($tabla, $fechaInicial, $fechaFinal, $id){
		...
		if($fechaInicial == null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE cobrador = $id");
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE fecha like '%$fechaFinal%' && cobrador = $id");
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE cobrador = $id && fecha BETWEEN '$fechaInicial' AND '$fechaFinal2'");
			$stmt -> execute();
